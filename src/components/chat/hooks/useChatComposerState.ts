...
export function useChatComposerState({
  ...
  // Guards against double-forking when a fork is in flight but the target
  // provider has not yet emitted its new session id (and routing has not yet
  // switched `selectedSession` to the new fork). Reset whenever the active
  // session changes (fork landed → navigated, or user switched sessions).
  ...
  const handleSubmit = useCallback(
    async (
      ...
      // --- Fork detection (§5.2 three-state) ---
      // When the user switched the tool (provider) on an EXISTING session, this
      // send is a fork: serialize the source session's history on the backend
      // and spawn the target provider as a brand-new session (no sessionId). The
      // new id is generated by the target tool and announced via session_created.
      const baseProviderForFork = selectedSession?.__provider as LLMProvider | undefined;
      const isForkAction =
        Boolean(effectiveSessionId) &&
        Boolean(baseProviderForFork) &&
        provider !== baseProviderForFork &&
        !forkInFlightRef.current;

      if (isForkAction && effectiveSessionId && baseProviderForFork) {
        forkInFlightRef.current = true;
        // Clear the active id so routing waits for the target tool's new id and
        // we never resume the old (foreign-provider) session under the new tool.
        ...
        sendMessage({
          ...
          options: {
            ...
            sourceProvider: baseProviderForFork,
            model: currentModel,
            // A fork is always an explicit model/provider choice.
            explicitModel: true,
            toolsSettings,
